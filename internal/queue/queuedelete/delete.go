...
// Item is an alias for queue.DeleteItem for backward compatibility
type Item = queue.DeleteItem

const bucketName = "queue_delete"
const dbPath = "data/queue.db"

// Queue wraps the generic queue implementation
type Queue struct {
	*queue.GenericQueue[*queue.DeleteItem]
}

// New creates a new delete queue instance
func New() (*Queue, error) {
	genericQueue, err := queue.NewGenericQueue[*queue.DeleteItem](dbPath, bucketName)
	if err != nil {
		return nil, err
	}
