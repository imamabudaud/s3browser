package queueunpublish

import (
	"s3browser/internal/queue"
)

// Item is an alias for queue.UnpublishItem for backward compatibility
type Item = queue.UnpublishItem

const bucketName = "queue_unpublish"
const dbPath = "data/queue.db"

// Queue wraps the generic queue implementation
type Queue struct {
	*queue.GenericQueue[*queue.UnpublishItem]
}

// New creates a new unpublish queue instance
func New() (*Queue, error) {
	genericQueue, err := queue.NewGenericQueue[*queue.UnpublishItem](dbPath, bucketName)
	if err != nil {
		return nil, err
	}
