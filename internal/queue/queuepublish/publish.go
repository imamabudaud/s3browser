package queuepublish

import (
	"s3browser/internal/queue"
)

// Item is an alias for queue.PublishItem for backward compatibility
type Item = queue.PublishItem

const bucketName = "queue_publish"
const dbPath = "data/queue.db"

// Queue wraps the generic queue implementation
type Queue struct {
	*queue.GenericQueue[*queue.PublishItem]
}

// New creates a new publish queue instance
func New() (*Queue, error) {
	genericQueue, err := queue.NewGenericQueue[*queue.PublishItem](dbPath, bucketName)
	if err != nil {
		return nil, err
	}
